{inputs, ...}: {
  ...
  in {
    packages = {
      difft = inputs.wrapper-modules.lib.wrapPackage (_: {
        inherit pkgs;
      ...
      # No difftastic settings: the minimal Legion toolbox uses this instead of `git`.
      git-minimal = inputs.wrapper-modules.wrappers.git.wrap {
        inherit pkgs settings;
      };
      git = inputs.wrapper-modules.wrappers.git.wrap {
        inherit pkgs;
        settings =
          settings
          // {
