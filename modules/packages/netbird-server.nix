{
  perSystem = {pkgs, ...}: {
    packages = {
      ...
      # server shipped as netbirdio/netbird-server:0.73.2 (chart image).
      # nixpkgs has no such component: its netbird package only knows
      # about client/ui/upload/management/signal/relay/proxy, and the
      # legacy `management` component uses a materially different state
      # layout than the unified server. The unified server instead lives
      # at `combined/` in the netbird monorepo (confirmed present at the
      # nixpkgs-pinned tag v0.74.3, which is >= the chart's 0.73.2):
      ...
      # same chart-style config.yaml the deployment uses.
