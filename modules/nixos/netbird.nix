{
  ...
  in {
    options.netbird = {
      client.enable = lib.mkEnableOption "Enable the netbird client";
      management.enable = lib.mkEnableOption "Enable the netbird management server and other required services";
    };
    config = {
      netbird.client.enable = lib.mkDefault true;
      services = {
        caddy.virtualHosts."netbird" = lib.mkIf config.netbird.management.enable rec {
        ...
        authelia.instances.main = lib.mkIf config.netbird.management.enable {
      ...
      systemd.services = lib.mkIf config.netbird.management.enable {
        netbird-management = {
      ...
      users = lib.mkIf config.netbird.management.enable {
        users.netbird = {
          isSystemUser = true;
          group = "netbird";
        };
      ...
      sops.secrets = lib.mkIf config.netbird.management.enable {
