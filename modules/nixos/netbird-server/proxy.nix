{self, ...}: {
  # docs/MIGRATION.md piece 3.2: NetBird reverse proxy (`netbird-proxy`) for
  # legion-node2, receiving the edge's raw SNI passthrough for
  # proxy.jeiang.dev/*.proxy.jeiang.dev (modules/nixos/edge/default.nix).
  # Sibling file to modules/nixos/netbird-server/default.nix (piece 3.1):
  # same node, same service boundary, imported under the same optional
  # pattern (modules/hosts/legion/default.nix, gated on the inventory
  # placing `netbird-proxy` -- modules/hosts/legion/_service-inventory.nix).
  flake.nixosModules.netbird-proxy = {
    ...
    # TLS strategy evidence (docs/MIGRATION.md Confirmed Decisions,
    # PREFERRED path): the nixpkgs-pinned netbird-proxy binary (v0.74.3,
    ...
    # fullchain.pem/key.pem straight to the proxy; the fallback TLS-ALPN-01
    # built-in-ACME path from the plan is unused. The proxy's file watcher
    # picks up renewals without a restart, so no reload hook is wired
  ...
  in {
    ...
    # State decision (docs/MIGRATION.md piece 3.2): unlike piece 3.1's
    # /mnt/netbird, this module mounts nothing on the
    # /mnt/netbird-proxy Volume the inventory pre-declared at piece 0.1
    # (modules/hosts/legion/_service-inventory.nix) in anticipation of the
    # built-in-ACME fallback. Static externally-provisioned certs (above)
    # mean the proxy itself keeps zero required persistent state: its ACME
    # material lives in security.acme's own node-local
    # /var/lib/acme/${certName}, reissued via DNS-01 on every reinstall --
    # wildcard DNS-01 has generous Let's Encrypt rate limits, so losing
    # that state costs a few seconds of reissuance, not a rate-limit
    # incident. docs/runbooks/netbird-migration.md (piece 3.3) does not
    # list /mnt/netbird-proxy as a prerequisite to attach for this reason.
    sops = {
      secrets = {
        # NetBird reverse-proxy access token (`nbx_...`), generated by
        # `netbird-server token create` against the local management
        # server (docs/runbooks/netbird-migration.md). Declared here even
        # though the value doesn't exist in secrets.yaml yet -- sops-nix
        # only checks at activation time, same precedent as
        # modules/nixos/netbird-server/default.nix's secrets.
        "netbird/proxy-token" = {};
        ...
        # value (docs/runbooks/edge-cutover.md already calls this out).
    ...
    systemd.services.netbird-proxy = {
      ...
      after = [
        ...
        # Registers against the local management server (3.1); Wants, not
      ...
      environment = {
        ...
        # Loopback on the same node as the server (piece 3.1's
        # config.yaml `server.listenAddress` is plain-HTTP ":80", so
        # http:// here is correct, not a downgrade).
        ...
        # Wired unconditionally, unlike 1.3's edge.crowdsec.enable toggle:
        # enforcement here is per-service, chosen in the NetBird dashboard
        ...
        # (`DenyCrowdSecUnavailable`). docs/runbooks/netbird-migration.md
        # carries the same "start in observe" guidance the chart's README
        # already gave operators.
        ...
        # placement table's "free custom ports" capability,
        # docs/MIGRATION.md); nothing to set here. Custom published-service
        # ports are opened by the operator per service (NetBird dashboard
